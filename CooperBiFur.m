...
x(:,1,1) = [1 0.05 0.1 0.5];%initial values [x y z w]
 b=0.5; cm=0.45; d=1;e=1;fp=0.5; gm=2;hp=0.11; k1=2; k2=0.2; N1=3; N2=3; %define parameters
for k = 1:length(a)
    for i = 1:N
        for j= 1:4
    ...
    pks = findpeaks(squeeze(x(1,1,:)));%find peaks of x
        plot(a(k),pks(round(0.2*length(pks)):length(pks)),'k.')%only plot last 50% of points
